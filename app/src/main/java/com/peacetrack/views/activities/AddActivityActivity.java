...
public class AddActivityActivity extends ActionBarActivity implements
    ...
    private EditText titleEditText,descriptionEditText,activityOutputEditText,associateOutputEditText;
    ...
    public void onResume() {
        ...
        if (cohortList.size() > 0) {
            ...
            titleEditText = (EditText) findViewById(R.id.activityName);
            descriptionEditText = (EditText) findViewById(R.id.activityDescription);
            activityOutputEditText = (EditText) findViewById(R.id.activityOutputs);
            associateOutputEditText = (EditText) findViewById(R.id.associatedoutputdata);
    ...
    private void saveActivity(){
        ...
        if (dateEditText.getText().length() == 0) {
            Toast.makeText(this,
                    getString(R.string.datecheck), Toast.LENGTH_SHORT)
                    .show();
            return;
        }
        ...
        if (timeEditText.getText().length() == 0) {
            Toast.makeText(this,
                    getString(R.string.timecheck), Toast.LENGTH_SHORT)
                    .show();
            return;
        }
        ...
        activity.setRelevantCohort(cohortSpinner.getSelectedItem().toString());
        activity.setDate(dateEditText.getText().toString());
        activity.setTime(timeEditText.getText().toString());
        activity.setActivityOutput(activityOutputEditText.getText().toString());
        activity.setAssociateOutputData(associateOutputEditText.getText().toString());

        ActivityDAO activityDAO = new ActivityDAO(getApplicationContext());
        activityDAO.addActivity(activity);
        finish();

        Intent intentt = new Intent(AddActivityActivity.this,
                AllActivitiesActivity.class);
        AddActivityActivity.this.startActivity(intentt);
    }
    ...
    private boolean checkExistingActivity(String name) {
        ...
        for (int i = 0; i < allActivity.size(); i++) {
            if (allActivity.get(i).getTitle().equalsIgnoreCase(name)) {
                return true;
            }
        }
        return false;
