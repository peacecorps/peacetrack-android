...
public class Indicators {
	 ...
	 * DATABASE_CREATE = "create table if not exists " + INDICATORS_TABLE + "("
	 * + COLUMN_ID + " integer primary key autoincrement," + COLUMN_POST +
	 * " text, " + COLUMN_SECTOR + " text, " + COLUMN_PROJECT + " text, " +
	 * COLUMN_GOAL + " text, " + COLUMN_OBJECTIVE + " text, " + COLUMN_INDICATOR
	 * + " text" + ");";
	 * 
	 * // used to create the table public static void onCreate(SQLiteDatabase
	 * database) { try { database.execSQL(DATABASE_CREATE); } catch
	 * (SQLException sqlException) { throw new
	 * Error("Error in creating database "); }
	 * 
	 * }
	 * 
	 * // used to upgrade the table public static void onUpgrade(SQLiteDatabase
